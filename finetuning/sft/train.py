...
class TrainingArguments(transformers.TrainingArguments):
    ...
    use_qlora: bool = field(default=False, metadata={"help": "Enable QLoRA 4-bit quantization training."})
    bnb_4bit_quant_type: str = field(default="nf4", metadata={"help": "Quantization type: nf4 or fp4."})
    bnb_4bit_compute_dtype: str = field(default="bfloat16", metadata={"help": "Compute dtype: bfloat16 or float16."})
...
def train():
    ...
    if training_args.use_qlora:
        compute_dtype = torch.bfloat16 if training_args.bnb_4bit_compute_dtype == "bfloat16" else torch.float16
        ...
        logging.info(f"QLoRA enabled: 4-bit quantization with {training_args.bnb_4bit_quant_type}")
    ...
    model = transformers.AutoModelForCausalLM.from_pretrained(
        ...
        torch_dtype=torch.bfloat16 if training_args.use_qlora else None,
    )

    # QLoRA: prepare model for kbit training
    if training_args.use_qlora:
        model = prepare_model_for_kbit_training(model)

    if training_args.use_peft or training_args.use_qlora:
